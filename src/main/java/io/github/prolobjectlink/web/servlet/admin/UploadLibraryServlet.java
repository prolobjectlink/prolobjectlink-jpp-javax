...
@WebServlet
public class UploadLibraryServlet extends UploadServlet implements Servlet {
	...
	protected void proccessfile(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			for (FileItem item : items) {
				...
				uploadLibrary(req, resp, uploadedFile.getAbsolutePath());
...
// .getMessage());
		...
		Template template = loader.load("/io/github/prolobjectlink/web/html/libraries.html");
		TemplateContext context = new TemplateContext();

		List<LibraryEntry> libraries = listLibraries();
		long size = librariesSize();

		// variables
		context.set("libraries", libraries);
